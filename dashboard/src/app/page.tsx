    ...
    <section className="mb-8 rounded-xl border border-white/10 bg-slatepanel/70 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <StatusPill ok={store.durable}>
            {store.durable ? "durable store" : "memory store"}
          </StatusPill>
        </div>
      </div>
      ...
      <dl className="mt-5 grid gap-3 text-sm text-mist md:grid-cols-2">
        ...
        <div className="rounded-md bg-black/25 p-3">
          ...
          <dd>
            {store.backend === "redis"
              ? "Upstash Redis is configured for durable review and quiz state."
              : "Using in-memory storage. Data resets when the process restarts."}
          </dd>
        </div>
        <div className="rounded-md bg-black/25 p-3">
...
export default async function HomePage({ searchParams }: Props) {
  ...
  return (
    <main>
      ...
      <SetupStatusPanel auth={auth} store={storeStatus} />
