...
2. **AUTONOMOUS:** Create or update the PR without waiting. Prefer `ManagePullRequest` in Cursor cloud agents; otherwise `gh pr create` / `gh pr edit`.

   Body must include Summary, Design Decisions, Changes, How to Verify, References to:
   - The Ledger (`architecture_and_roadmap.md`)
   - `$ARGUMENTS/design.md` and `$ARGUMENTS/plan.md`
   - Note that QRSPI was run under Autonomous Mode

3. **Report the PR URL** in the agent summary.
